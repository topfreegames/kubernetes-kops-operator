...
func TestLaunchTemplateResourceExists(t *testing.T) {
	RegisterFailHandler(Fail)
	g := NewWithT(t)
	...
	testCases := []struct {
		description    string
		resourceName   string
		tfFileContent  string
		expectedResult bool
		expectedError  bool
	}{
		{
			description:  "should return true when launch template resource exists",
			resourceName: "nodes-arm64-infra-test-us-east-1-k8s-tfgco-com",
			tfFileContent: `
resource "aws_launch_template" "nodes-arm64-infra-test-us-east-1-k8s-tfgco-com" {
  name = "nodes-arm64.infra-test.us-east-1.k8s.tfgco.com"
}
`,
			expectedResult: true,
		},
		{
			description:  "should return false when launch template resource does not exist",
			resourceName: "non-existent-resource",
			tfFileContent: `
resource "aws_autoscaling_group" "nodes-arm64-infra-test-us-east-1-k8s-tfgco-com" {
  name = "nodes-arm64.infra-test.us-east-1.k8s.tfgco.com"
}
`,
		...
		{
			description:    "should return false when directory has no tf files",
			resourceName:   "nodes-arm64-infra-test-us-east-1-k8s-tfgco-com",
			tfFileContent:  "",
			expectedResult: false,
		},
		{
			description:  "should handle resource names with special characters",
			resourceName: "nodes.test-cluster.k8s.local",
			tfFileContent: `
resource "aws_launch_template" "nodes.test-cluster.k8s.local" {
  name = "nodes.test-cluster.k8s.local"
}
`,
			expectedResult: true,
		},
	...
	for _, tc := range testCases {
		t.Run(tc.description, func(t *testing.T) {
			tmpDir, err := os.MkdirTemp("", "test_launch_template")
			g.Expect(err).NotTo(HaveOccurred())
			defer os.RemoveAll(tmpDir)

			if tc.tfFileContent != "" {
				err = os.WriteFile(fmt.Sprintf("%s/kubernetes.tf", tmpDir), []byte(tc.tfFileContent), 0644)
				g.Expect(err).NotTo(HaveOccurred())
			}

			exists, err := LaunchTemplateResourceExists(tmpDir, tc.resourceName)
			if tc.expectedError {
				g.Expect(err).To(HaveOccurred())
			} else {
				g.Expect(err).NotTo(HaveOccurred())
